...
public sealed class LocalizationService : INotifyPropertyChanged
{
    ...
    private static IReadOnlyDictionary<string, string> BuildChineseResources()
    {
        return new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["Header.Subtitle"] = "只切换认证和模型路由，不碰本地线程库。",
            ["Common.Chinese"] = "中文",
            ...
            ["Section.ThreadLibraryValue"] = "不会被修改（sessions / session_index / state_5.sqlite / thread_history_1.sqlite）",
            ...
            ["Dialog.ApiEditor.UseOpenAiThreadView.Checkbox"] = "通过内置 openai provider 路由 API，并沿用同一个本地线程库",
    ...
    private static IReadOnlyDictionary<string, string> BuildEnglishResources()
    {
        return new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["Header.Subtitle"] = "Only switches authentication and model routing, never the local thread library.",
            ["Common.Chinese"] = "中文",
            ["Common.English"] = "English",
            ...
            ["Section.ThreadLibraryValue"] = "Will not be modified (sessions / session_index / state_5.sqlite / thread_history_1.sqlite)",
            ...
            ["Dialog.ApiEditor.UseOpenAiThreadView.Checkbox"] = "Route the API through the built-in openai provider and keep the same local thread library",
