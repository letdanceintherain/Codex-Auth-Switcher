...
public sealed class LocalizationService : INotifyPropertyChanged
{
    ...
    private static IReadOnlyDictionary<string, string> BuildChineseResources()
    {
        return new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["Section.ThreadLibraryValue"] = "仅同步未归档对话；保留聊天内容与对话 ID，跳过归档。",
    ...
    private static IReadOnlyDictionary<string, string> BuildEnglishResources()
    {
        return new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["Section.ThreadLibraryValue"] = "Sync unarchived conversations only; preserve messages and thread IDs. Archives are skipped.",
